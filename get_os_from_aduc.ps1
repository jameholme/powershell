#gets computers from aduc and gathers their os information

$ad_path = #change your path. example: "OU=,OU=,OU=,DC=,DC=,DC=,DC="
$computer_list = get-adcomputer -filter * -searchbase $path #uncomment if you only want so many-># | select -first 150
$save_path = #change your path. example: "C:\Path\Path\file.csv"
